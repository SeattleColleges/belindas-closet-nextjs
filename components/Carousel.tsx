...
const CarouselElement = React.forwardRef<HTMLDivElement, CarouselProps>(function Stat(inProps, ref) {
    ...
    <ResponsiveWrapper>
      <CarouselComponentWrapper>
        ...
        <CarouselContainerWrapper>
          ...
          <CarouselContainer id={carouselID}>{children}</CarouselContainer>
...
declare module "@mui/material/styles" {
  interface Components {
    CarouselElement?: {
      styleOverrides?: Partial<OverridesStyleRules<CarouselElementClassKey, "CarouselElement", Theme>>;
    };
  }
}
    ...
    <Stack direction="row" spacing={2} sx={{ width: "100%", justifyContent: "center" }}>
      <ThemeProvider theme={theme}>
        <CarouselElement carouselID={carouselID} products={products} title={title}>
          {products && products.map((product, index) => <SimpleItemPreview product={product} key={index} />)}
        </CarouselElement>
      </ThemeProvider>
    </Stack>
  );
